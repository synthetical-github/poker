"""
Erzeugt Kandidaten-Templates fuer Hero-Karten aus allen 52 Screenshots.
WICHTIG: Speichert in separatem Ordner (nicht direkt in aktive Templates),
um Fehl-Labels zu vermeiden.
"""
import csv
import os
import cv2
...
def main() -> None:
    ...
    out_dir = 'assets/hero_card_templates_candidates'
    os.makedirs(out_dir, exist_ok=True)

    report_path = 'hero_candidates_report.csv'

    # Schwellwerte fuer Kandidaten (konservativ)
    min_score = 0.90
    max_gap = 0.02
    ...
    for i in range(1, 53):
        ...
        for ci, roi in enumerate(layout['hero_hole_cards']):
            ...
            context = f'hero{ci + 1}'
            surf = det._extract_hero_card_surface(crop, context)
            ...
            for card_name, tmpl_list in det.hero_card_templates.items():
            ...
            out_name = (
                f'{best_name}__SS{i:02d}_hero{ci + 1}_'
                f's{best_score:.3f}_g{gap:.3f}.png'
            )
            out_path = os.path.join(out_dir, out_name)

            if candidate:
                cv2.imwrite(out_path, surf)
                saved += 1
